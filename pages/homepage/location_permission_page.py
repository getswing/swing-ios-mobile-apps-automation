from locators.location_permission_locators import LocationPermissionLocators as L
from pages.base_page import BasePage


class LocationPermissionPage(BasePage):
    ROOT_LOCATOR = L.TXT_DESCRIPTION
    PAGE_NAME = "LocationPermissionPage"

    def verify_screen(self):
        self.wait_until_loaded()
        assert self.is_visible(L.TXT_DESCRIPTION, timeout=20), "Location permission screen not shown"
        assert self.is_visible(L.BTN_ENABLE, timeout=5), "Enable location button not shown"
        assert self.is_visible(L.BTN_LATER, timeout=5), "I'll do it later button not shown"
        self.capture_step("Verify location permission page")
        return self
