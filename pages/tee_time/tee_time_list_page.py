from locators.tee_time.tee_time_list_locators import TeeTimeListLocators as L
from pages.base_page import BasePage


class TeeTimeListPage(BasePage):
    ROOT_LOCATOR = L.TXT_TITLE
    PAGE_NAME = "TeeTimeListPage"

    def verify_screen(self, member_type=""):
        self.wait_until_loaded()
        assert self.is_visible(L.TXT_TITLE, timeout=20), "Tee time list screen not shown"
        assert self.is_visible(L.IMG_SEARCH, timeout=5), "Tee time search field not shown"
        assert self.is_visible(L.EL_COUNTRY, timeout=5), "Tee time country selector not shown"
        if member_type == "swing-pass":
            assert self.is_visible(L.IMG_SWING_PASS_FILTER, timeout=5), "Tee time Swing Pass filter not shown"
        self.capture_step("Verify tee time list page")
        return self
